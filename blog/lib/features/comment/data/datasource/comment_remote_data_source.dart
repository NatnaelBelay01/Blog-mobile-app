...
class CommentRemoteDataSourceImpl implements CommentRemoteDataSource{
  ...
  Future<List<CommentModel>> loadComments(String blogId) async {
		try {
			...
			final res = await supabase.from("comments").select('*').eq('blog_id', blogId);
			final commentList = res.map((value) => CommentModel.fromJson(value)).toList();
			return commentList;
		} catch (e) {
  ...
  Future<CommentModel> uploadComment(CommentModel commentModel) async {
		try {
			final res = await supabase.from('comments').insert(commentModel.toJson()).select();
